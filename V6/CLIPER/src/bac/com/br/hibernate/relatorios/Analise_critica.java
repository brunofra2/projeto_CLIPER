 ...
 * @author bruno
 */
public class Analise_critica {

    public Analise_critica(List fevereiro, List marco,List abril,List maio,String imagem) {
        try {
            Connection conexao = Utils.open();
            HashMap parametro = new HashMap();
            parametro.put("fevereiro", fevereiro);
            parametro.put("marco", marco);
            parametro.put("abril", abril);
            parametro.put("maio", maio);
            if(imagem.contains("home")){
                parametro.put("imagem", imagem+"/treinamento/mineiro.png");
            }else{
                parametro.put("imagem", imagem+"\\mineiro.png");
            }
            InputStream caminhorelatorio = getClass().getClassLoader().getResourceAsStream("bac/com/br/hibernate/relatorios/Analise critica.jasper");
